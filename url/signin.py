...
def get_containerid(url):
    ...
    #print(content)
    #print(content.get('data').get('tabsInfo'))
...
def get_userInfo(id):
    ...
    content=json.loads(data)
    #print(content)
    profile_image_url=content.get('data').get('userInfo').get('profile_image_url')
    description=content.get('data').get('userInfo').get('description')
    profile_url=content.get('data').get('userInfo').get('profile_url')
    verified=content.get('data').get('userInfo').get('verified')
    guanzhu=content.get('data').get('userInfo').get('follow_count')
    name=content.get('data').get('userInfo').get('screen_name')
...
if __name__=="__main__":
    file="C:\\Users\\83443\\Desktop\\lxy.txt"
    get_userInfo(id)
    get_weibo(id,file)
